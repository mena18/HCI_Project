  ...
  const api_url = 'http://127.0.0.1:8000/test/download_operations'
...
return a - b  ;
    ...
    var level_duplicate ;
    var count = 1;                                               // to make (n) duplicate
    //***************************************************************************************************************************
    //***************************************** END **********************************************************
    //*************************************************************************************************************************** -
    //------------------------sortedData_deadline ---------------------------------
       ...
       if (currentlevel === level_duplicate){
         ...
         console.log(count);
            ...
            if(parseFloat(currentlevel) == parseFloat(level_duplicate) -1)
            {
              ...
              leveldata.push(parseFloat(((count - count_null)/count)*100)); //cannot subtract must stored in varaiable let count_deadline.length sortedData_deadline.length
              count = 1;
              count_null = 0 ;
            }
      ...
      for(var i=0 ;i<sortedData_finished.length;i++){
        if(sortedData_finished[i].finished==null)
        {
          ...
          if(parseFloat(cols[1]) === parseFloat(years_duplicate)){
            ...
            {
              const type_check = sortedData_finished[i].type;
              if(type_check == 1)
              {
                Type1++;
              }
